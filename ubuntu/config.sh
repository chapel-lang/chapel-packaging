...
configHelp() {
    # Print help
    echo;
    echo "This script builds the rpm for a given Ubuntu release"
    echo "It expects \$CHPL_RELEASE to be defined and \$UBUNTU_RELEASE to be"
...
configSetup() {
    ...
    if [ -z ${UBUNTU_RELEASE} ]; then
        echo "\$UBUNTU_RELEASE is not defined"
        echo "Define via command line argument or environment variable"
        exit 1
    fi
